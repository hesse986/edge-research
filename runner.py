...
EDGE_MAP = {
    ...
    "short_term_reversal_liquidity":  pe.short_term_reversal_liquidity,
    "trend_following":                pe.trend_following,
    "oi_price_divergence":            pe.oi_price_divergence,
    "cross_exchange_premium":         pe.cross_exchange_premium,
    ...
    "feargreed_extreme":        edgemod.edge_feargreed_extreme,
    "wick_pressure":            edgemod.edge_wick_pressure,
    "funding_confluence":       edgemod.edge_funding_confluence,
...
# ============================================================
with open("asset_groups.yaml") as f:
    ASSET_GROUPS = yaml.safe_load(f)
...
def test_asset(asset, hypo, tf, cost, min_trades, max_hold, direction="both"):
    ...
    try:
        ...
        if hypo["edge"] == "feargreed_extreme":
            feargreed = datamod.load_feargreed(VALIDATION_START, VALIDATION_END, use_cache=True)
